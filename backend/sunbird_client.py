...
def summarise_text(text: str) -> str:
    ...
    payload = {"text": text}
    response = requests.post(url, json=payload, headers=_get_headers(), timeout=600)
...
def translate_text(text: str, target_language: str) -> str:
    ...
    Uses Chat Inference endpoint with message format.
    ...
    # Use Chat Inference format with system + user messages
    payload = {
        "messages": [
            {
                "role": "system",
                "content": f"You are a helpful assistant who translates English text into {lang_name}. Return ONLY the translated text, nothing else."
            },
            {
                "role": "user",
    ...
    print(f"[DEBUG] Translation payload being sent: {payload}")
    response = requests.post(url, json=payload, headers=_get_headers(), timeout=600)
    ...
    # Chat Inference returns content directly: {"content": "...", "model_type": "...", ...}
    translated = data.get("content", "")
